...
- (void)viewDidLoad {
    ...
    _closeButton = UIButton.button.WH_setImage_forState(WOOImageNamed(@"ico_close"),UIControlStateNormal).WH_frame(CGRectMake(10, 10, 40, 40));
    @weakify(self)
    ...
    if (photoView.item.finished) {
        ...
        [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
            ...
            if (status==PHAuthorizationStatusAuthorized) {
                UIImage * image = [self.imageManager imageFromMemoryForURL:photoView.item.imageUrl];
                dispatch_async(dispatch_get_main_queue(), ^{
...
-(void)openThePrivacyWithTitle:(NSString*)title{
    [[WOOAlertTool shareInstance]showAlterViewWithMessage:title andDoneBlock:^(UIAlertAction * _Nonnull action) {
